...
describe('Acl', () => {
  it('should work deprecated with single principal, action & securable', function () {
    const acl = new Acl()
  ...
  it('should work with single principal, action & securable', function () {
    ...
    acl.permit({ principal, securable, action })

    expect(acl.permits({ principals: principal, actions: action, securable })).to.equal(true)
    expect(acl.denies({ principals: principal, actions: action, securable })).to.equal(false)

    acl.unpermit({ principal, securable, action })

    expect(acl.permits({ principals: principal, actions: action, securable })).to.equal(false)
    expect(acl.denies({ principals: principal, actions: action, securable })).to.equal(false)
  ...
  it('should work deprecated with single principal, action & no securable', function () {
    const acl = new Acl()
    const principal = uuid()
    const action = uuid()
  ...
  it('should work with single principal, action & no securable', function () {
    ...
    acl.permit({ principal, action })

    expect(acl.permits({ principals: principal, actions: action })).to.equal(true)
    expect(acl.denies({ principals: principal, actions: action })).to.equal(false)
  })

  it('should work deprecated with multiple principals, actions & a securable', function () {
    const acl = new Acl()
  ...
  it('should work with multiple principals, actions & a securable', function () {
    ...
    acl.permit({ principal: principals[0], securable, action: actions[0] })
    acl.permit({ principal: principals[1], securable, action: actions[1] })

    expect(acl.permits({ principals: principals[0], actions: actions[0], securable })).to.equal(true)
    expect(acl.permits({ principals: principals[1], actions: actions[1], securable })).to.equal(true)
    expect(acl.permits({ principals, actions, securable })).to.equal(true)
    expect(acl.denies({ principals, actions, securable })).to.equal(false)

    acl.unpermit({ principal: principals[1], securable, action: actions[1] })

    expect(acl.permits({ principals: principals[0], actions: actions[0], securable })).to.equal(true)
    expect(acl.permits({ principals: principals[1], actions: actions[1], securable })).to.equal(false)
    expect(acl.permits({ principals, actions, securable })).to.equal(false)
    expect(acl.denies({ principals, actions, securable })).to.equal(false)

    acl.unpermit({ principal: principals[0], securable, action: actions[0] })

    expect(acl.permits({ principals: principals[0], actions: actions[0], securable })).to.equal(false)
    expect(acl.permits({ principals: principals[1], actions: actions[1], securable })).to.equal(false)
    expect(acl.permits({ principals, actions, securable })).to.equal(false)
    expect(acl.denies({ principals, actions, securable })).to.equal(false)
